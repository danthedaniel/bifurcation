...
export const FULL_RES_STEPS_PER_CHUNK = 256;
...
// A framebuffer with a single state texture attached as COLOR_ATTACHMENT0.
...
export class BifurcationRenderer {
  ...
  static create(canvas: HTMLCanvasElement): BifurcationRenderer | null {
    ...
    try {
      return new BifurcationRenderer(gl);
    } catch (e) {
  ...
  private createStateTarget(): StateTarget {
    ...
    const state = this.createStateTexture();
  ...
  private allocateTierTextures(tier: TierCache, width: number, height: number) {
    ...
    for (const target of tier.targets) {
      gl.bindTexture(gl.TEXTURE_2D, target.state);
      // prettier-ignore
  ...
  // Blit a region from `src` to `dst` (single color attachment). Uses NEAREST
  ...
  private shiftRetainedRegion(tier: TierCache, dx: number, dy: number) {
    ...
    this.blitState(A, B, srcX, srcY, w, h, dstX, dstY);
    ...
    this.blitState(B, A, dstX, dstY, w, h, dstX, dstY);

    gl.bindFramebuffer(gl.FRAMEBUFFER, null);
  }

  // Write initial state into the current target. When `rects` is provided,
  // only those regions are written (scissored); otherwise the full frame is
  // initialized.
  private runInitPass(
    tier: TierCache,
  ...
  // writing into the other one. `iterationBase` is the global iteration index
  // of the first step in this chunk. When `rects` is provided, only those
  // regions are written (scissored) so retained texels are never re-stepped.
  private runStepPass(
    tier: TierCache,
    uniforms: ShaderUniforms,
    steps: number,
    ...
    gl.bindTexture(gl.TEXTURE_2D, source.state);
    gl.uniform1i(locations.u_state, 0);
    gl.uniform2f(locations.u_sim_resolution, tier.simSize[0], tier.simSize[1]);
    gl.uniform2f(locations.u_size, uniforms.size[0], uniforms.size[1]);
    gl.uniform2f(locations.u_center, uniforms.center[0], uniforms.center[1]);
    gl.uniform1i(locations.u_steps, steps);
    gl.uniform1i(locations.u_iteration_base, iterationBase);
  ...
  private runDisplayPass(
    ...
    gl.uniform1i(locations.u_step_count, stepCount);
  ...
  startRender(uniforms: ShaderUniforms, options: RenderOptions) {
    ...
    if (tier.complete && tier.renderedUniforms) {
      ...
      if (uniformsMatchExceptCenter(cached, uniforms)) {
        ...
        if (Math.abs(dx) >= width || Math.abs(dy) >= height) {
          ...
          this.runDisplayPass(tier, uniforms.stepCount);
    ...
    const renderChunk = () => {
      ...
      if (steps > 0) {
        // stepsDone IS the iteration base for this chunk: the retained region
        // is already complete at totalSteps, and the exposed strips run their
        // own 0 -> totalSteps scoped by rects.
        this.runStepPass(tier, effectiveUniforms, steps, stepsDone, rects);
        stepsDone += steps;
      }
  ...
  previewView(uniforms: ShaderUniforms): boolean {
    ...
    this.runDisplayPass(bestTier, uniforms.stepCount, uvScale, uvOffset);
    return true;
  }
